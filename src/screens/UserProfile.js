...
const UserProfilePage = () => {
	...
	const [showFollow, setShowFollow] = useState(state ? !state.Following : null);

	const config = axiosConfig(localStorage.getItem("jwt"));

	useEffect(() => {
		axios.get(`https://server-instagram.herokuapp.com/user/${userid}`, config).then((res) => {
			setData(res.data);
		});
	...
	const followUser = () => {
		axios.put(`https://server-instagram.herokuapp.com/follow`, { followId: userid }, config).then((result) => {
			dispatch({
	...
	const unfollowUser = () => {
		axios.put(`https://server-instagram.herokuapp.com/unfollow`, { unfollowId: userid }, config).then((result) => {
			dispatch({
				type: UPDATE_FOLLOW_DATA,
				payload: { Followers: result.data.Followers, Following: result.data.Following },
			});
