...
const Home = () => {
	...
	const likePost = (id) => {
		axios.put(`https://server-instagram.herokuapp.com/like`, { postId: id }, config)
			.then((result) => {
	...
	const unlikePost = (id) => {
		axios.put(`https://server-instagram.herokuapp.com/Unlike`, { postId: id }, config)
			.then((res) => {
				const newData = data.map((item) => {
	...
	const bookmark = (id) => {
		axios.put(`https://server-instagram.herokuapp.com/bookmark-post`, { postId: id }, config)
			.then((result) => {
				dispatch({
					type: BOOKMARK_POST,
	...
	const removeBookmark = (id) => {
		axios.put(`https://server-instagram.herokuapp.com/remove-bookmark`, { postId: id }, config)
			.then((result) => {
				dispatch({
					type: BOOKMARK_POST,
					payload: { Bookmarks: result.data.Bookmarks },
	...
	const makeComment = (text, postId) => {
		setComment("");
		axios.put(`https://server-instagram.herokuapp.com/comment`, { text, postId }, config)
			.then((result) => {
				const newData = data.map((item) => {
					if (result.data._id === item._id) return result.data;
					else return item;
	...
	const deletePost = (postId) => {
		axios.delete(`https://server-instagram.herokuapp.com/deletepost/${postId}`, config).then((res) => {
